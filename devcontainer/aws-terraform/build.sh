...
## Prerequisite: Make sure you set secret enviroment variables in CI
## DOCKER_USERNAME=nnthanh101
## DOCKER_PASSWORD=
...
build() {
  echo "helm latest, hold the release candidates ..."
  ...
  terraform_version=$(curl -s https://api.github.com/repos/hashicorp/terraform/releases | jq -r '.[].tag_name | select(startswith("v"))' \
    | sort -rV | head -n 1 |sed 's/v//')
  echo "terraform version is $terraform_version"

  echo "Packer latest ..."
  packer_version=$(curl -s https://api.github.com/repos/hashicorp/packer/releases | jq -r '.[].tag_name | select(startswith("v"))' \
    | sort -rV | head -n 1 |sed 's/v//')
